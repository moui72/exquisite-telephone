...
- [ ] T002 [defect: f333f65c] [parallel] In `server/src/socket/handlers.test.ts`'s existing `describe('onVoteToPlayAgain', ...)` block, write a failing test asserting `onVoteToPlayAgain` rejects with `{ error: 'room-not-in-reveal' }` when called while `Room.status` is `lobby` or `writing`, and that `Room.playAgainVotes` is unmodified in that case. Confirm the test fails against current `onVoteToPlayAgain`, then add the identical `room.status !== 'reveal'` guard to `onVoteToPlayAgain` in `server/src/socket/handlers.ts`.
