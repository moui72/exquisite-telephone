...
describe('sanitizeForDisplay (datamodel.md Consolidated view sanitization)', () => {
  it('strips C0 control characters and ESC (0x1B)', () => {
  ...
  it('removes bidi override characters (U+202A–202E, U+2066–2069)', () => {
    expect(sanitizeForDisplay('a‪b‮c⁦d⁩e')).toBe('abcde');
  });

  it('removes zero-width characters (U+200B–200D, U+FEFF)', () => {
    expect(sanitizeForDisplay('a​b‌c‍d﻿e')).toBe('abcde');
  });

  it('leaves ordinary text, punctuation, and emoji unchanged', () => {
