...
describe('CoverDecorationCanvas', () => {
  it.fails("renders the pre-stamped \"<username>'s book\" plaque and a drawing canvas", () => {
  ...
  it.fails('updates the client-local draft via onOpsChange on pointer up (no per-stroke emit)', () => {
  ...
  it.fails('honors monochromeOnly by hiding the color palette (passed through to DrawingCanvas)', () => {
  ...
  it.fails('shows the color palette when monochromeOnly is false', () => {
